...
class Admin extends Base {
	 ...
	 *
	 * @since 1.0
	 * @since 3.17.7 Directs to the setup wizard instead of the settings page
	 */
	public function settings_page_redirect() {
    	if( get_option( 'codesigner-activated' ) != 1 ) {
			update_option( 'codesigner-activated', 1 );
    		wp_safe_redirect( admin_url( 'admin.php?page=codesigner_setup' ) );
			exit();
    	}
	}
	...
	public function admin_notices() {
		...
		// Halloween 2023 starts
		if( isset( $_GET['dismiss'] ) && $_GET['dismiss'] == 'cd-halloween_23' ) {
			update_option( '_codesigner-halloween_2023', date_i18n( 'U' ) );
		}
		if ( strtotime( '07-11-2023' ) > date_i18n( 'U' ) && '' == get_option( '_codesigner-halloween_2023' ) && ! defined( 'CODESIGNER_PRO' ) ) {
			printf(
				'<div class="notice notice-success is-dismissible cx-hlw-notice">
					<div id="cx-text"><p>%1$s <mark id="cx-timer"></mark></p></div>
					<div id="cx-btn"><p><a class="button button-primary" href="%3$s" target="_blank">%2$s</a></p></div>
					<div id="cx-dismiss"><p><a id="cx-dismiss-link" href="%4$s">%5$s</a></p></div>
				</div>',
				__( '<strong>Happy Halloween!</strong> Up to <mark>40% DISCOUNT</mark> on <strong>CoDesigner Pro</strong>. Offer ends in' ),
				__( 'Grab the Deal Now' ),
				'https://codexpert.io/codesigner/pricing?utm_source=cd-halloween_23',
				add_query_arg( [ 'dismiss' => 'cd-halloween_23' ] ),
				__( 'I don\'t want this' ),
			);

			?>
			<script>
			    function updateTimer() {
			        const targetDate = new Date("2023-11-07T00:00:00");
			        ...
			        document.getElementById("cx-timer").innerHTML = `${days}d ${hours}h ${minutes}m ${seconds}s`;
			    }

			    setInterval(updateTimer, 1000);

			    document.getElementById("cx-dismiss-link").addEventListener("click", function(event) {
			        if (!confirm("Are you sure you want to miss the deal?")) {
